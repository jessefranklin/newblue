...
?>
  ...
  function event_tax_save_changes(){
	...
	// END Intel: Update additional location fields
      ...
      $term_name = esc_attr(stripslashes($term_name));
      $term = term_exists( $term_name, $tax );
      if($term !== 0 && $term !== null){
        $taxtermID = (int)$term['term_id'];
        wp_set_object_terms( $eventid, $taxtermID, $tax );
      }else{
      ...
      if($taxtermID){
        ...
        foreach($fields as $key=>$value){
          ...
          if(isset($_POST[$value['var']])){
            ...
            do_action('evo_tax_save_each_field',$value['var'], sanitize_text_field($_POST[$value['var']]) );


            if($value['var']=='location_address'){
              if(isset($_POST['location_address']))
              $latlon = eventon_get_latlon_from_address(sanitize_text_field($_POST['location_address']) );
              ...
              $term_meta['location_address' ] = (isset($_POST[ 'location_address' ]))? sanitize_text_field($_POST[ 'location_address' ]):null;

              continue;
            }


            $term_meta[ $value['var'] ] = str_replace('"', "'", sanitize_text_field($_POST[$value['var']]) );
        ...
        wp_set_object_terms( $eventid, $taxtermID, $tax , false);
    ...
    'status'=>$status,
    'content'=>$content,
    'htmldata'=> EVO()->evo_admin->metaboxes->event_edit_tax_section($tax , $eventid )
    )); exit;
  }
  ...
  function event_tax_remove(){
    ...
    if(!empty($_POST['termid'])){
      wp_remove_object_terms( $eventid, $termid, $tax, false);
      $status = 'good';
      $content = __('Changes successfully saved!','eventon');
    }else{
      $content = __('Term ID was not passed!','eventon');
