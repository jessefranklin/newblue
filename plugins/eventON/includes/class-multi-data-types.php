...
class evo_mdt{
  ...
  function evomdt_ajax(){
    ...
    switch($type){
      ...
      'content' =>$this->mdt_form($event_id, $tax),
      'status'=>'good'
      )); exit;
      break;
      case 'editform':
      ...
      //$term_id = (int)$_POST['termid'];
      echo json_encode(array(
      'content' =>$this->mdt_form($event_id, $tax, $term_id),
      'status'=>'good'
      )); exit;
      break;
      case 'save':
      ...
      if(!empty($_POST['mdt'])){
        ...
        $result = wp_set_object_terms($event_id, $mdts, $tax, false);
      }else{
        $result = wp_set_object_terms($event_id, '', $tax, false);
      }
      echo json_encode(array(
      'result'=>$result,
      'content'=>$this->get_mdt_display_list($event_id, $tax),
  ...
  function content($name, $tax, $x){
    global $post;
    $p_id = empty($_REQUEST['post'])? 0 : intval($_REQUEST['post']);
    $p_id = empty($p_id)? $post->ID : $p_id;
