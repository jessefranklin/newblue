			...
			// do not do anything
				...
				// SLIDE DOWN eventcard
					...
					if(click_item.hasClass('open')){
						...
						click_item.parent().removeClass('dark-text');
						click_item.parent().css('background-color', 'transparent');
					}else{
						// accordion
						if(accord__=='1'){
						...
						click_item.parent().addClass('dark-text');
						click_item.parent().css('background-color', '#f5f5f5');
					}
	...
	// ==============================
		...
		// selection on filter dropdown list
			$('.eventon_filter_dropdown').on('click','p',function(){
				...
				// restyling of filter section for intel new blue connect
				$(this).parent().parent().children('.filtering_set_val').removeClass('menuArrow');
...
// Search Scripts
	...
	$(document).on('click', function(event) {
    if ($(event.target).parents('#evo_search').length == 0){
      $('body').find(".evo_search_results").hide();
    }
	});
	...
	// Submit search from search box
		...
		function do_search_box(OBJ){
			...
			Evoresults = OBJ.closest('#page').children('.site-content-contain').find('.evo_search_results');
			Evocount = OBJ.closest('#page').children('.site-content-contain').find('.evo_search_results_count');
			...
			$.ajax({
				...
				success:function(data){
					...
					if(Evosearch.find('.no_events').length>0){
						...
						// find event count
						$('body').find(".evo_search_results").show();
