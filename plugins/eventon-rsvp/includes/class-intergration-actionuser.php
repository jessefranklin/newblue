...
class evors_actionuser{
		...
		function fields($field, $event_id, $default_val, $EPMV, $opt2, $lang){
			...
			echo "<div class='row evors'><p>";
				...
				echo $helper->html_yesnobtn(array(
					'id'=>'evors_rsvp',
					'input'=>true,
					'label'=>evo_lang_get('evoAUL_rsvp1', 'Allow user registration capabilities for this event', $lang, $opt2),
			...
			echo "<div id='evors_rsvp_section' class='row evoau_sub_formfield' style='display:".($evors_rsvp?'':'none')."'>
					<p class='label evoau_total_capacity' style='padding:8px 0'><label>".evo_lang_get('evoAUL_rsvp2','Total Available Registration Capacity',$lang, $opt2)."</label><input type='text' class='' style='margin-left:5px;' name='evors_capacity_count' value='".$evors_capacity_count."' placeholder=''/></p>";
				...
				echo $helper->html_yesnobtn(array(
					'id'=>'evors_show_rsvp',
					'input'=>true,
					'label'=>evo_lang_get('evoAUL_rsvp4', 'Show registration count for the event on EventCard', $lang, $opt2),
				...
				echo $helper->html_yesnobtn(array(
					'id'=>'evors_max_active',
					'input'=>true,
					'label'=>evo_lang_get('evoAUL_rsvp9', 'Limit maximum capacity count per each registration', $lang, $opt2),
				...
				echo $helper->html_yesnobtn(array(
					...
					'input'=>true,
					'label'=>evo_lang_get('evoAUL_rsvp8', 'Receive email notification for new registrations', $lang, $opt2),
	...
	// event manager additions
		...
		function event_manager_row($event_id, $EPMV){
			if( evo_check_yn($EPMV,'evors_rsvp') ){
				echo "<a class='evoauem_additional_buttons load_rsvp_stats' data-eid='{$event_id}'>".evo_lang('View Registration Stats')."</a>";
			}
		}
		...
		function evors_ajax_get_auem_stats(){
			...
			$html = $this->event_manager_show_data(sanitize_text_field($_POST['eid']) );
